# Plan

## Active

### B19A — Settings / fullscreen / camera repair
- ⏳ unify slide background rendering pipeline
- ⏳ wire Settings darken + blur into actual slide rendering
- ⏳ prevent full-slide transition zoom from stacking with camera/focus zoom
- ⏳ fix 1-based step focus indexing for `CameraStage`
- ⏳ harden fullscreen clipping and stage containment
- ⏳ add proposal example with right-side image
- ⏳ add camera/focus-region example for proposal or companion slide
- ⏳ update RCA memory after implementation + validation

### B19 — Lint & CI polish
- ⏳ theme-token contrast rule
- ⏳ deck export-zip CLI (deck + assets)
- ⏳ CI workflow (GitHub Actions running `lint-deck.ts`)
- ⏳ per-slide `sound` schema validation
- ⏳ LintPanel "Copy as JSON" button

## Completed

### B18 (partial — 6/10)
- ✅ Audio overhaul (HTMLAudioElement, stop-before-play, public/sounds/)
- ✅ Whoosh → fade_swoosh_v4 for slide jumps
- ✅ Zoom sound for step transitions + milestone clicks
- ✅ Initial Settings → background-color attempt (`ThemeWrap` override) — superseded by B19A RCA because it did not handle per-slide backgrounds, image mode, darken, blur, or fullscreen/camera layering
- ✅ `.hl` text-shadow spec applied (crisp ink-stamp)
- ✅ Clickable StepsSlide rows + TimelineSlide milestones
- ✅ LintPanel severity filter + group-by-slide
- ✅ `lint-deck.ts` glob support
- ✅ music-url-not-https + music-volume-out-of-range rules
- ⚠️ Fullscreen overflow fix was incomplete — B19A owns the real root-cause repair

### B17
- ✅ 9 advanced lint rules + CLI `--json`

### B16
- ✅ Lint foundation + LintPanel
