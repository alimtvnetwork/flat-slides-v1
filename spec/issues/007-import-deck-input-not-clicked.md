...
- 2026-06-06 — fixed. Import handlers now reuse the mounted `SettingsDrawer` file input and trigger `click()` synchronously from the button handler; `pickJsonFile` still supports a temporary DOM-mounted fallback for non-drawer callers.
